...
RUST_TARGET_JSON = config/$(RUST_TARGET).json
...
MAKE_CONGIGDIR =  $(MAKE_BASEDIR)config/
...
iso:
	$(MAKE) kernel
	-$(MKDIR) $(MAKE_IMGDIR) $(MAKE_TMPDIR)grub-iso/boot/grub/
	$(CP) $(MAKE_BINDIR)kernel.elf $(MAKE_TMPDIR)grub-iso/boot/
